...
def warp_pc_to_image(
    ...
    warped = cv2.warpPerspective(
        pc_color, H_grid_to_image, (img_w, img_h),
...
def draw_metric_bar(frame: np.ndarray, pc_in_box: float, pc_at_ball: float) -> None:
    ...
    cv2.putText(frame, f"PC in box: {pc_in_box:.2f}", (bar_x + bar_w + 10, bar_y + 24),
    ...
    cv2.putText(frame, f"PC at ball: {pc_at_ball:.2f}", (bar_x2 + bar_w + 10, bar_y + 24),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
...
def project_ball_to_image(
    ball_x_m: float, ball_y_m: float, H_world_to_image: np.ndarray
...
def render_clip(
    ...
    for fi in frame_indices:
        ...
        cv2.putText(frame, f"{clip_id} | {action} | frame {fi}",
