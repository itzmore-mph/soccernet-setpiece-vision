...
def run_tvcalib() -> dict:
    ...
    # CPU is the only bit-reproducible path: mps/cuda reductions are not
    # deterministic, which shifts homographies and every downstream stat.
    # Default to cpu for the canonical run; override with TVCALIB_DEVICE for
    # faster (non-reproducible) experimentation.
    device = os.getenv("TVCALIB_DEVICE", "cpu")
    cmd = [
        str(tvcalib_python),
        str(tvcalib_script),
        "--images_path",
