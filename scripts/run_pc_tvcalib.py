"""Phase 4 pitch control under TVCalib detections, mirroring nb03 exactly.

Reads:
    outputs/detections_pipeline_tvcalib.parquet
    outputs/ball_positions.parquet

Writes:
    outputs/pitch_control_tvcalib.parquet  (track='tvcalib')

All Laurie Shaw / Spearman params identical to nb03 — locked.
"""
...
def main():
    det = pd.read_parquet(OUTPUTS_DIR / "detections_pipeline_tvcalib.parquet")
    balls = pd.read_parquet(OUTPUTS_DIR / "ball_positions.parquet")
    print(f"detections_tvcalib: {det.shape}  |  ball_positions: {balls.shape}")
    pc = process_track(det, track_name="tvcalib", team_col="team_kmeans", balls=balls)
    out = OUTPUTS_DIR / "pitch_control_tvcalib.parquet"
    pc.to_parquet(out, engine="pyarrow", index=False)
    print(f"pitch_control_tvcalib frames: {len(pc)}  |  saved: {out}")
