...
Funnel stages:
    ...
    22 clips with autonomous ball position (ball-detection coverage)
    674 pipeline PC frames
    662 paired frames (pipeline ∩ GT)
    22 matched clips (clip-level inferential unit)
    ~24-26 effective independent observations (ICC design effect)
...
def compute_stages() -> list[tuple[str, str, float]]:
    ...
    return [
        ...
        ("Clips with ball position", "autonomous ball detection (67%)", n_ball_clips),
        ...
        ("Effective observations", "ICC design effect ~26", round((neff_lo + neff_hi) / 2)),
    ], (neff_lo, neff_hi)


def render(stages: list[tuple[str, str, float]], neff_range: tuple[float, float]) -> Path:
    n = len(stages)
