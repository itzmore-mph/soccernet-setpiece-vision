"""Optimized pipeline: single video pass combining Fixes 2, 3, 4.
...
def run_optimized_clip(
    ...
    """Single video pass combining Fixes 2, 3, 4.
    ...
    if ball_raw_detections:
        ...
        try:
            ...
            ball_position_df = pd.DataFrame([{
                ...
                "n_interpolated": int(ball_df["interpolated"].sum()) if "interpolated" in ball_df.columns else 0,
            }])
        except ValueError:
...
def main() -> None:
    ...
    weights_path = hf_hub_download(
        repo_id=SOCCANA_REPO, filename=SOCCANA_WEIGHTS_PATH_IN_REPO
    )
    yolo_player = YOLO(weights_path)
    yolo_ball = YOLO(weights_path)
    print(f"Soccana weights: {weights_path}  device: {DEVICE}")
    print("  yolo_player: classes=[0,2], conf=0.25, TTA, agnostic_nms")
    print("  yolo_ball:   classes=[1], conf=0.15, ByteTrack")
    ...
    for i, (_, clip) in enumerate(clips.iterrows()):
        det_rows, ball_df, reason = run_optimized_clip(
            clip, yolo_player, yolo_ball, H_lookup
        )
    ...
    if all_ball_positions:
        ...
        ball_df = pd.DataFrame(
            columns=["split", "clip_id", "action_class", "x_pitch", "y_pitch",
                     "n_raw_detections", "n_interpolated"]
        )
