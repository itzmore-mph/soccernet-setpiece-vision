...
YOLO_CONF = 0.25
# Soccana (Adit-jain/soccana) was trained at 1280x1280 ("yolov11_sahi_1280").
# ultralytics defaults to imgsz=640, which downscales a 1080p broadcast frame
# ~3x and pushes small objects (the ball above all) below the conf threshold.
# Running inference at the training resolution improves small-object recall and,
# combined with dropping TTA, brings player counts closer to ground truth.
# Note: the model was also trained with SAHI sliced inference; we do NOT tile
# here because SAHI does not carry ByteTrack state (tracking is load-bearing for
# team assignment and ball interpolation).
YOLO_IMGSZ = 1280
# Default to CPU for bit-reproducible runs: mps/cuda float reductions are not
# deterministic, which drifts YOLO confidences around the conf threshold, flips
# boxes in/out, and shifts every downstream stat. Override with TORCH_DEVICE=mps
# for faster (non-reproducible) experimentation only, never for committed runs.
PIPELINE_SEED = 10  # matches TVCalib seed


def resolve_device() -> str:
    """Return the torch device string, honouring TORCH_DEVICE env override.

    Auto-detects CUDA -> MPS -> CPU so a machine without MPS (Windows/Linux)
    falls back to cpu rather than raising. Degrades to cpu if torch is
    unavailable. This is an opt-in helper: the canonical ``DEVICE`` below stays
    cpu so committed runs are bit-reproducible (see the Determinism section in
    CLAUDE.md). Call ``resolve_device()`` explicitly only for throwaway,
    non-reproducible speed runs on an accelerator.
    """
...
# Canonical device: cpu unless TORCH_DEVICE explicitly overrides. Deliberately
# NOT resolve_device(): auto-detecting mps on Apple Silicon would silently make
# committed runs non-deterministic.
DEVICE = os.getenv("TORCH_DEVICE", "cpu")


def set_deterministic(seed: int = PIPELINE_SEED) -> None:
    """Pin all RNG and force deterministic kernels for byte-reproducible runs.

    Call once at the start of any pipeline entry point. On CPU the forward pass
    is bit-deterministic; the seed is hygiene (YOLO inference uses little RNG).

    Parameters
    ----------
    seed : int
        Seed applied to ``random``, ``numpy``, and ``torch`` RNGs.
    """
    import random
    ...
    # warn_only: YOLO ops without a deterministic kernel won't crash; on CPU
    # they are deterministic anyway.
...
# HSV thresholds for referee-jersey detection (OpenCV HSV: H in [0,179], S/V in [0,255])
_REF_HUE_LO, _REF_HUE_HI = 20, 65  # yellow-to-green band (typical referee kit)
_REF_SAT_MIN = 80  # saturated colour (rules out grey/white kits)
_REF_VAL_MAX = 50  # very dark jersey (black referee kit)


def _is_ref_like(c: np.ndarray) -> bool:
    """Return True if an HSV centroid looks like a referee kit.

    A cluster is treated as referees when it is either a saturated yellow-to-green
    (the common high-vis kit) or very dark (a black kit).

    Parameters
    ----------
    c : np.ndarray
        Shape (3,): mean [H, S, V] centroid (OpenCV HSV ranges).

    Returns
    -------
    bool
        True if the centroid matches the referee-kit heuristic.
    """
    h, s, v = float(c[0]), float(c[1]), float(c[2])
    if _REF_HUE_LO <= h <= _REF_HUE_HI and s > _REF_SAT_MIN:
        return True
    if v < _REF_VAL_MAX:
        return True
    return False
...
def track_frame(
    ...
    """
    # TTA (augment=True) kept: it raises real-player recall (position-matched to
    # GT: recall 0.94 with TTA vs 0.90 without), and Pitch Control is recall-
    # driven — missing real players removes legitimate PC mass and degrades GT
    # agreement. imgsz=1280 supplies the precision (fewer false positives) that
    # TTA alone lacked. Class-agnostic NMS merges overlapping player/referee boxes.
    res = yolo.track(
        source=image_bgr,
        conf=YOLO_CONF,
        imgsz=YOLO_IMGSZ,  # match the model's 1280 training resolution
        classes=[player_class, referee_class],
        tracker="bytetrack.yaml",
        persist=True,
        device=DEVICE,
        verbose=False,
        augment=True,  # TTA - boosts recall of real players (PC needs recall)
        agnostic_nms=True,  # class-agnostic NMS (merge player/referee overlaps)
    )[0]
    ...
    ids = res.boxes.id.cpu().numpy().astype(int) if res.boxes.id is not None else np.full(len(conf), -1, dtype=int)
    x = xyxy[:, 0]
    y = xyxy[:, 1]
    w = xyxy[:, 2] - xyxy[:, 0]
    h = xyxy[:, 3] - xyxy[:, 1]
...
def validate_ball_positions(
    ...
    merged["euclidean_error"] = np.sqrt(
        (merged["x_auto"] - merged["x_gt"]) ** 2 + (merged["y_auto"] - merged["y_gt"]) ** 2
    )
...
def filter_pitch_bounds(coords: np.ndarray) -> np.ndarray:
    ...
    if len(coords) == 0:
        return coords
    mask = (
        (coords[:, 0] >= 0) & (coords[:, 0] <= PITCH_LENGTH_M) & (coords[:, 1] >= 0) & (coords[:, 1] <= PITCH_WIDTH_M)
    )
    return coords[mask]
...
# DEPRECATED: superseded by run_optimized_clip in run_optimized_pipeline.py.
...
def run_clip(
    ...
    if track_ids:
        track_mean_hsv = np.array([np.nanmean(track_hsv_samples[tid], axis=0) for tid in track_ids])
...
# Pre-computed grid and pitch-zone masks (static — do not recompute per frame)
_PC_XS = np.linspace(0.0, PITCH_LENGTH_M, GRID_NX)
_PC_YS = np.linspace(0.0, PITCH_WIDTH_M, GRID_NY)
_PC_XX, _PC_YY = np.meshgrid(_PC_XS, _PC_YS)
_PC_GRID = np.column_stack([_PC_XX.ravel(), _PC_YY.ravel()])  # (GRID_NX*GRID_NY, 2)
_PEN_LEFT = (_PC_XX <= 16.5) & (_PC_YY >= 13.84) & (_PC_YY <= 54.16)
_PEN_RIGHT = (_PC_XX >= PITCH_LENGTH_M - 16.5) & (_PC_YY >= 13.84) & (_PC_YY <= 54.16)
_THIRD_LEFT = _PC_XX <= PITCH_LENGTH_M / 3
_THIRD_RIGHT = _PC_XX >= 2 * PITCH_LENGTH_M / 3
...
def split_attack_defend(
    ...
    bx, by = ball_xy
    teams = [
        t for t in np.unique(team_labels) if t is not None and t != -1 and not (isinstance(t, float) and np.isnan(t))
    ]
    if len(teams) < 2:
...
def summarise_surface(pc: np.ndarray, ball_xy: tuple) -> dict:
    ...
    if (ny, nx) == _PC_XX.shape:
        pen_left, pen_right = _PEN_LEFT, _PEN_RIGHT
        third_left, third_right = _THIRD_LEFT, _THIRD_RIGHT
    else:
        # Non-default grid (e.g. a 1x1 cell): masks must match pc.shape, so
        # rebuild them for this resolution rather than index with the static masks.
        ...
        third_left = xx <= PITCH_LENGTH_M / 3
        third_right = xx >= 2 * PITCH_LENGTH_M / 3
    if bx < PITCH_LENGTH_M / 2:
        att_third = third_left
        att_box = pen_left
    else:
        att_third = third_right
        att_box = pen_right
    return {
...
def expand_balls_to_frames(balls: pd.DataFrame, det: pd.DataFrame) -> pd.DataFrame:
    """Expand per-clip ball positions to per-frame rows matching process_track schema.

    Handles both schemas:
    - Old: already has frame_idx + ball_x_m + ball_y_m columns → returned as-is.
    - New: one row per clip with x_pitch + y_pitch → broadcast to every frame in det.
    """
    if "frame_idx" in balls.columns and "ball_x_m" in balls.columns:
        return balls
    frames = det[["split", "clip_id", "frame_idx"]].drop_duplicates()
    renamed = balls.rename(columns={"x_pitch": "ball_x_m", "y_pitch": "ball_y_m"})
    renamed = renamed[["split", "clip_id", "ball_x_m", "ball_y_m"]]
    return frames.merge(renamed, on=["split", "clip_id"], how="inner")
        ...
        out.append(
            {
                ...
                "n_attackers": int(len(att_xy)),
                "n_defenders": int(len(def_xy)),
                "ball_x_m": ball_xy[0],
                "ball_y_m": ball_xy[1],
                **m,
            }
