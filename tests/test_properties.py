...
def test_n_eff_formula(n, m, icc):
    ...
    if icc == 0.0:
        assert abs(n_eff - n) < 1e-10, f"When ICC=0, n_eff should equal n. Got n_eff={n_eff}, n={n}"

    # When ICC = 1, n_eff = n / m (maximum correlation → maximum reduction)
    if icc == 1.0:
        expected = n / m
        assert abs(n_eff - expected) < 1e-10, f"When ICC=1, n_eff should equal n/m={expected}. Got n_eff={n_eff}"
...
def test_pitch_bounds_filter(coords):
    ...
    if len(arr) > 0:
        valid_mask = (arr[:, 0] >= 0) & (arr[:, 0] <= 105) & (arr[:, 1] >= 0) & (arr[:, 1] <= 68)
        expected_count = int(valid_mask.sum())
        assert len(result) == expected_count, f"Expected {expected_count} valid coords in output, got {len(result)}"
    ...
    for i in range(len(frame_indices) - 1):
        ...
        if gap <= 5:
            for f in range(frame_indices[i] + 1, frame_indices[i + 1]):
                assert f in result["frame_idx"].values, f"Frame {f} should be interpolated (gap={gap} <= 5)"
        elif gap > 5:
            for f in range(frame_indices[i] + 1, frame_indices[i + 1]):
                assert f not in result["frame_idx"].values, f"Frame {f} should NOT be interpolated (gap={gap} > 5)"
...
def test_ball_position_median_fallback(frames_1_to_5):
    ...
    assert abs(result[0] - expected_x) < 1e-10, f"Expected median x={expected_x}, got {result[0]}"
    assert abs(result[1] - expected_y) < 1e-10, f"Expected median y={expected_y}, got {result[1]}"
...
def test_soccernet_data_rejects_invalid(path_suffix):
    ...
    assert exit_code != 0 or isinstance(exit_code, str), f"Expected non-zero exit code, got {exit_code!r}"
    # The error message should contain the path so the user knows what's wrong
    if isinstance(exit_code, str):
        assert (
            fake_path in exit_code or "gamestate-2024" in exit_code
...
_PRIVATE_PARQUETS_PRESENT = (OUTPUTS_DIR / "detections_soccana_tvcalib.parquet").exists() and (
    OUTPUTS_DIR / "ball_positions.parquet"
).exists()


@pytest.mark.skipif(
    not _PRIVATE_PARQUETS_PRESENT,
    reason="private video-derived parquets not present (gitignored); run on local dataset only",
)
def test_pc_idempotence():
    ...
    if "frame_idx" in balls_raw.columns and "ball_x_m" in balls_raw.columns:
        ...
        balls_renamed = balls_raw.rename(columns={"x_pitch": "ball_x_m", "y_pitch": "ball_y_m"})
    ...
    pc_run1 = process_track(det, track_name="soccana_tvcalib", team_col="team_kmeans", balls=balls)
    pc_run2 = process_track(det, track_name="soccana_tvcalib", team_col="team_kmeans", balls=balls)
...
def test_load_tvcalib_lookup_skips_non_finite(tmp_path: Path):
    ...
    def _row(split, clip_id, frame_idx, mat):
        ...
        return {"split": split, "clip_id": clip_id, "frame_idx": frame_idx, **dict(zip(cols, flat))}
